...
(function () {
    ...
    ************************************/
    ...
    var efcmanager = {}
        ...
        , defaultOptions = {
            ...
            cometInterval : 20000,      //default polling interval
    ...
    ************************************/
        ...
        , comet : function() {
            ...
            cometRequestHandler = $.ajax({
                ...
                success : function(response) {
                    ...
                    //timestamp4display = cometData[0].timestamp
    ...
    function updateDigital(cometObj, price, lastPrice, changeFlag, change, changePercent, high, low, suspended,timestamp) {
        ...
        if(suspended){
            container.removeClass('up down').addClass('suspended')
            container.find('.price-icon').hide();
            priceDiffPercent = "0.00%";
            priceDiff='0.00'
        }

        container.find('.price-current').html(numeral(price).format(priceFormat));
        container.find('.dayrange-low').html(numeral(low).format(priceFormat));
        container.find('.dayrange-high').html(numeral(high).format(priceFormat));
        ...
        var timestamp4display = timestamp || cometTimestamp;
        container.find('.lastupdate').html(moment(timestamp4display * 1000).format(options.dateFormat));
    }
    ...
    var defautEvents = {
        ...
        'cometed' : function(event) {
            ...
            for(i in cometData) {
                ...
                symbols[cometData[i].symbol + 'high'] = cometData[i].high;
                symbols[cometData[i].symbol + 'low'] = cometData[i].low;
                symbols[cometData[i].symbol + 'suspended'] = cometData[i].suspended;
                symbols[cometData[i].symbol + 'timestamp'] = cometData[i].timestamp;

            }

            if(lastCometData.length < 1) {
                for(symbol in symbols) {
                    root.trigger('pricechanged', [symbol, symbols[symbol], false, symbols[symbol+ 'changeFlag'], symbols[symbol+ 'change'],
                        symbols[symbol+ 'changePercent'],symbols[symbol+ 'high'],symbols[symbol+ 'low'],symbols[symbol+'suspended'],symbols[symbol+'timestamp']]);
                }
                return false;
            }
            ...
            for(symbol in symbols) {
                ...
                if(typeof backup[symbol] === 'undefined') {
                    this.trigger('pricechanged', [symbol, symbols[symbol], false, symbols[symbol+ 'changeFlag'], symbols[symbol+ 'change'],
                        symbols[symbol+ 'changePercent'],symbols[symbol+ 'high'],symbols[symbol+ 'low'],symbols[symbol+'suspended'],symbols[symbol+'timestamp']]);
        ...
        'pricechanged' : function(event, symbol, price, lastPrice, changeFlag, change, changePercent,high,low,suspended,ts) {
            ...
            for(i in cometPool) {
                ...
                if(cometObj.symbol == symbol) {
                    updateDigital(cometObj, price, lastPrice , changeFlag, change, changePercent,high,low,suspended,ts);
                }
            }
...
// +----------------------------------------------------------------------
// + Datetime: 14-6-9 下午6:13
// +----------------------------------------------------------------------
// + 行情颜色切换器，依赖 jquery 和 jquery.cookie.js
// +----------------------------------------------------------------------
...
(function (root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        exports.QuoteColorSwitchover = factory($);

        // 浏览器全局对象
    } else {
        root.QuoteColorSwitchover = factory(root.$);
    }
     ...
     * @constructor
     */
    var QuoteColorSwitchover = function (_options) {
        var self = this;

        var options = _options ? _options : {};
        self._selector = options.selector ? options.selector : '';
        self._cookieName = options.selector ? options.cookieName : 'quote.color';

        self._initialize();
    }
    $.extend(QuoteColorSwitchover.prototype, /* @lends QuoteColorSwitchover.prototype */ {
        /**
         * @var {Object} 常量
         */
        CONSTS: {
            // 绿涨红跌（Type Green Up Red Down）
            TYPE_GURD: 1,
            // 红涨绿跌（Type Red Up Green Down）
            TYPE_RUGD: 2
        },
        ...
        _bindEvents: function() {
            var self = this;
            $('.J_switchover-gurd').click(function() {
                self.setType(self.CONSTS.TYPE_GURD);
            });
            $('.J_switchover-rugd').click(function() {
                self.setType(self.CONSTS.TYPE_RUGD);
            });

        },
        /**
         * 根据 cookie 为图表适配合适的参数
         * @param chartOptions
         * @returns {Object}
         */
        adaptiveChartOptions: function(chartOptions) {
            var self = this;

            // 目前仅支持 light 和 dark 主题
            if(chartOptions.chartTheme == 'light' || chartOptions.chartTheme == 'dark') {
                if(self.getType() == self.CONSTS.TYPE_RUGD) {
                    chartOptions.chartTheme = chartOptions.chartTheme + 'Rugd';
                }
            }
        ...
        render: function(type) {
            $('head').append('<link rel="stylesheet" href="/css/quote-color-type-' + type + '.css">');
        },
        /**
         * 设置类型并生效
         * @param {number} type
        ...
        setType: function(type) {
            ...
            // 默认为红涨绿跌
            if(type != self.CONSTS.TYPE_GURD && type != self.CONSTS.TYPE_RUGD) {
                type = self.CONSTS.TYPE_RUGD;
            }
            self.render(type);
            self._saveType(type);

            $('#chart').attr('src', $('#chart').attr('src'));
        },
        /**
         * 保存类型值到 cookie
         * @param type
        ...
        _saveType: function(type) {
            ...
            $.cookie(self._cookieName, type, {path: '/'});
        },
        ...
        getType: function() {
            ...
            var type =  $.cookie(self._cookieName);
            // 默认为红涨绿跌
            if(type != self.CONSTS.TYPE_GURD && type != self.CONSTS.TYPE_RUGD) {
                type = self.CONSTS.TYPE_RUGD;
                self._saveType(type);
