...
$row['msg']=html_entity_decode($row['msg']);
	if($row[tuid]!="ALL"){ 
		...
		$omsg="<div class='msg_li'><div style='clear:both;'></div><div class='msg' id='{$row['msgid']}'><div class='msg_head'><span class='time'>".date('H:i',$row['mtime'])."</span><span class='u'>{$row['uname']}</span><span class='dui'>对</span><span class='u'>{$row['tname']}</span><span class='shuo'>说</span></div>
		 <div class='msg_content' style='{$row[style]};'>{$row['msg']}</div></div></div>".$omsg;}
	}
	else{
		$omsg="<div class='msg_li'><div style='clear:both;'></div><div class='msg' id='{$row[msgid]}'><div class='msg_head'><span class='time'>".date('H:i',$row['mtime'])."</span><span class='u'> {$row[uname]}：</spa></div> <div class='msg_content' style='{$row[style]};'>{$row[msg]}</div></div></div>".$omsg;
	}
}
//其他处理
...
$db->query("insert into  {$tablepre}msgs(rid,ugid,uid,uname,tuid,tname,mtime,ip,msg,`type`,laiyuan)values('{$cfg[config][id]}','{$userinfo[gid]}','{$userinfo[uid]}','{$userinfo[username]}','{$cfg[config][defvideo]}','{$cfg[config][defvideonick]}','".gdate()."','{$onlineip}','手机进入直播间','3','{$laiyuan}')");
...
<body style="position: relative; top: 0px; background-image: url(./images/phone_bg.jpg);background-size: cover; overflow: hidden; background-position: initial initial; background-repeat: no-repeat no-repeat;">
<div id="details"></div>
...
<div id="shared"></div>
<article>
  <div id="logo"> <img src="images/m_logo.png" alt="" height="25px"> </div>
  <section id="#head_1">
...
<?=tohtml($cfg['config']['phonefp']);?>
    ...
    <nav>
      <ul>
        ...
        <li class="spec2">操作建议<span class=""></span> </li>
  ...
  <section>
    <div id="publicChat" class="publicChat" style="overflow-y: auto;display: block;" ><?=$omsg?></div>
    <div id="qqOnline" style="width: 100%; display: none;" class="white">
		<iframe src="hd.php" frameborder="0" scrolling-y="auto" width="99.9999%" height=100%></iframe>
    </div>
...
<div id="qqchat_win" style="display:none">
    <div class="qqchat_win" style="">
<?php
                         $time=mktime(0,0,0,date('m'),date('d'),date('Y'));
    $time=time()-$time;
$query=$db->query("select m.realname,ms.nickname from {$tablepre}members m,{$tablepre}memberfields ms  where m.uid=ms.uid  and m.gid=3 and m.realname!='' and ((ms.onlinestart<=ms.onlineend  and ms.onlinestart<=$time and  ms.onlineend>=$time) or ( ms.onlinestart>ms.onlineend and ms.onlinestart<=$time and  ms.onlineend<=$time) or ( ms.onlinestart>ms.onlineend and ms.onlinestart>=$time and  ms.onlineend>=$time))  order by m.lastactivity DESC  ");

while($row=$db->fetch_row($query)){
	$list.="<li><a href='http://wpa.qq.com/msgrd?v=3&amp;uin={$row['realname']}&amp;site=qq&amp;menu=yes' target='_blank'><img border='0' src='./images/qqjt.gif' alt='点击这里给我发消息' title='点击这里给我发消息' width='72px' height='22px'></a></li>";
}
echo $list;
?>
        <div class="def"><img src="./images/x_close.png" onclick="layer.close(pageii);"></div></div>
</div>
<script>OnInit();
$("#footer").on("click", ".redbagclose",
    function() { 
        $(this).parent().remove()
    });
</script>
