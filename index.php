...
// 房间对应的主播信息
$anchorinfo = $db->fetch_row($db->query("select * from {$tablepre}anchor  where concat(',',rooms,',') like '%,{$rid},%'"));
...
$room_admin=check_admin($_COOKIE['power']);
...
<html>
    <head>
        <meta charset="UTF-8">
        <title><?=$cfg['config']['title']?>_民间股神在线直播</title>
...
#jqr_caitiao{background:#fff;border:rgb(146,146,142) 1px solid;border-radius:3px;width:70px;height:115px;color:#000;z-index:99999999;position:absolute;}
#jqr_caitiao dd{background:#f8f6fe;height:21px; line-height:22px;text-align:center;cursor:pointer;}
#jqr_caitiao dd:hover{background:#E6E6E6;}
...
<div id="jqr_caitiao" style="display:none;"></div>
...
<div class="wbtns clearfix">
                                <a href="#" class="btn wbtn">首页</a>
                                <a href="#" class="btn wbtn">直播</a>
                                <a href="http://fxs.ifcde.com/" class="btn wbtn">股神投票</a>
                                <a href="#" class="btn wbtn">手机直播</a>
        ...
        <!-- 在线用户列表开始 -->
                  ...
                  <div  class="title_tab"> <a href="javascript:void(0)" class="bg_png2" onClick="bt_SwitchListTab('User')" id="listTab1">在线会员<font style="display: block;"></font></a>  
                
                     <a href="javascript:void(0)" style="display: none;" onClick="bt_SwitchListTab('Other')" id="listTab2"><?=($userinfo['gid']=='3'?"我的客户":"官方客服")?><font style="display: none;">(<span id="OnlineOtherNum"></span>)</font></a> 
                  </div>


                      <div style="clear:both"></div>
                      <div id="OnlineUser_Find" style=" width:182px;height:25px; margin:0px; padding:2px; overflow:hidden; line-height:25px; border:1px solid #999" class="bg_png2">
                      ...
                      <div id="OnLineUser" class="OnLineUser"  style="height:350px;">
                        <div id="group_my"></div>
                        <?=$groupli?>
  ...
  <div id="UI_Central"  class="bg_png">
    ...
    <marquee scrollamount="3" id="msg_tip_show">
       ...
       <?php
        ...
        while($row=$db->fetch_row($query)){
            echo "<span style='color:#000'>".tohtml($row[msg])."</span>";
        }
        ?>
    </marquee>
         </div>
...
?>
            ...
            <div class="send-msg f-12" id="UI_Control" class="tool_bar">
 ...
 <input  type="hidden" name="Personal" id="Personal" value="false" />
    <div  class="inputmsg">
      <div class="liuyan_huang">
          ...
          <div style=" width:30px; float:right;"><input name="Send_bt" type="image" class="btn btn-light-red sendmsg pull-right f-12" id="Send_bt" title="发送"/> </div>
      </div>
  ...
  <span id="bar_list" style="float:left; width:100%;">
        ...
        <a href="javascript:void(0)" title="表情" id="bt_face" class="bar_1 bar" onclick="showFacePanel(this,'#Msg');" isface="2"></a>
        <a href="javascript:void(0)" title="彩条" id="bt_caitiao" class="bar_3 bar"></a>
        <a href="javascript:void(0)" title="上传图片" id="bt_myimage" class="bar_3 bar" onclick="bt_insertImg('#Msg')"></a>
        <? if($room_admin) {?>
        <a href="javascript:void(0)" title="弹幕" id="bt_rollnotice" class="bar_4 bar" onclick="bt_rollnotice()"></a>
        <a href="javascript:void(0)" title="机器人" id="bt_automsg" class="bar_5 bar" onclick="bt_automsg()"></a>
        <a href="javascript:void(0)" title="机器人" id="bt_rebotsNum" class="bar_5 bar" onclick="bt_rebotsNum()"></a>
        <? }?>
                 <a href="javascript:void(0)" title="清屏" id="bt_qingping" class="bar_6 bar" onclick="bt_MsgClear();"></a>
         <a href="javascript:void(0)" title="锁屏" id="bt_gundong" class="bar_7 bar" select="true" onclick="bt_toggleScroll()"></a>
     ...
     <!--    <div class="tool_div">
                ...
                <a class="bar_8 bar" id="bt_myimage" onmouseout="_toolTimeRed()" onmouseover="_toolCloseTimeRed()" onclick="user_hb()">鲜花：<font id="redbags">0</font></a>
        </div> -->
       <!-- <div id="Y_pub_Tools">
        ...
        <span style="margin-left:5px; display:none"> 对<div id="ToUser" class="lts_chat5" data-id="ALL">大家</div><span style="position: relative;"><div id="apDiv2" style="display: none;"></div></span>&nbsp;说</span>
        <input type="hidden" name="Personal" id="Personal" value="false">
    </span>   
    ...
    <div id="manage_div">
    <? if($room_admin) {?>
      <span>公告：<a href="javascript:void(0)"  title="置顶公告" ><input type="checkbox" id="msg_tip"></a></span>
    <? }?>
      <select id="chat_type" style="display:none">
        ...
        &nbsp;&nbsp;
      <!-- <label><input type="checkbox" id="msg_tip">置顶公告</label>&nbsp;&nbsp; -->
...
<div class="f-13 shareinfo clearfix">
    ...
    <p class="pull-left">
        主播：<span class="hoster"><a href="#">一木</a></span> </p>
		  ...
		  <div class="take-attention pull-left">
              <?php 
			     // echo $rid;
                  ...
                  // echo $curid;
              if(in_array($curid, $rid) && isset($_COOKIE['user_roomid'])){

              ?>
               <button class="btn btn-light-red follow-someone  pull-left attentions" uid="12610441"><b>已关注</b><span><?=$cfg['config']['follows']+$cfg['config']['untruefollows']?></span>	</button>
              <?php
                  }else{ ?> 
                         <button class="btn btn-light-red follow-someone  pull-left attentions" uid="12610441"><b>+关注</b><span><?=$cfg['config']['follows']+$cfg['config']['untruefollows']?></span>	</button>
                  <?php }?>

               <button class="btn btn-light-red private-msg  pull-left"  uid="<?=$anchorinfo['id']?>" receiver="<?=$anchorinfo['name']?>" data-toggle="modal" data-target="#sendMsg">私信</button>
 ...
 <script type="text/javascript">
     ...
     var url = window.location.href,pos=url.lastIndexOf('/')+1; //末尾开始查找字符串'/'
     var roomid=parseInt(url.substring(pos));//截取房间号
   // console.log(roomid); 
   ...
   $(function(){
       ...
       $('.attentions').click(function(){
            if(aa==''||bb==''){
                   layer.msg('您还没有登录！',{shift: 6});return false;
            }else{
                             ...
                             // url: 'http://user.good100.top/api/user_follow',
                              ...
                              success: function(data){
                                          ...
                                          if(data.status==3){ //首次关注
                                                   ...
                                                   // layer.msg('已关注！',{shift: 6});return false;
                                                    //取消关注
                                                $.post('./include/follows.func.php?act=cancle',{'follows':1,'roomid':roomid},function(data){
                                         
                                                    if(data.status==false){return false;}else{
                                                         var follows=parseInt($('.follow-someone').children('span').html())-1;
                                                         $('.follow-someone').children('b').html('+关注');
                                                         var count = follows;
                                                         $('.follow-someone').children('span').html(count);
                                                     
                                                         // $.cookie('user_roomid',null,{path:'/'});
                                                    };          
                                                ...
                                                $.ajax({
                                                          ...
                                                          dataType: 'jsonp',
                                                          cache: false,
                                                          url: 'http://user.good100.top/api/cancel_roomid',
                                                          data: {'uid':aa, 'roomid':roomid,'act':'cancel'},
                                                          success:function(data){
                                                            console.log(data);
                                                          }
                                          ...
                                          $.post('./include/follows.func.php?act=sure',{'follows':1,'roomid':roomid},function(data){
                                             
                                            if(data.status==false){return false;}else{
                                                 var follows=parseInt($('.follow-someone').children('span').html())+1;
                                                 $('.follow-someone').children('b').html('已关注');
												 var count = follows;
                                                 $('.follow-someone').children('span').html(count);
                                                // $.cookie('user_roomid','0',{path:'/'});
                                            };           
                ...
                -->
                    ...
                    <li>
                        ...
                        <span class="appcode dropdown-menu" role="menu">
                            <i class="ico-arw-bottom"></i>  
                            <img src="<?=$cfg['config']['ewm']?>" alt="app扫码观看" width="101">
                        </span>
                    </li>
    ...
    <div>
        <div class="tab-content" style="display: block;">
            ...
            <div class="ad">
                <h4>官方活动</h4>
                <ul class="ad-list">
                  <li><a href="#"><img src="img/ad1.png" class="mCS_img_loaded"></a></li>
                  <li><a href="#"><img src="img/ad2.png" class="mCS_img_loaded"></a></li>
                  <li><a href="#"><img src="img/ad3.png" class="mCS_img_loaded"></a></li>
                </ul>
            </div>
        </div>  
          ...
          <div class="modal-body">
            <form class="form-horizontal">
                <div class="form-group f-12">
                    <div class="col-sm-3">
                        <input type="checkbox" value="1" name="feedback">
                        <label class="feedback-item">卡顿</label>
                    </div>
                    <div class="col-sm-3 no-padding black">
                        <input type="checkbox" value="2" name="feedback">
                        <label class="feedback-item">黑屏</label>
                    </div>
                    <div class="col-sm-3 no-padding black">
                        <input type="checkbox" value="3" name="feedback">
                        <label class="feedback-item">模糊</label>
                    </div>
                    <div class="col-sm-4 no-padding">
                        <input type="checkbox" value="4" name="feedback">
                ...
                <div class="form-group text-center">
                    ...
                    <input type="hidden" name="problem_uid" id="problem_uid" value="<?=$_COOKIE['uid']?>" />
                </div>
            </form>
          </div>
        </div>
...
<script>
    ...
    $("#myFeedback .btn").click(function(){
        watchApp.sendReport($("#myFeedback"),'feedback',$("#feedback-con"),'/room/UserCenter/addproblem.php');
    }); 
