...
case "reg":
        ...
        if ($db->num_rows($query))
            exit("<script>alert('用户名已经被使用!换一个，如{$u}2015');location.href='register.php'</script>");
        $query = $db->query("select uid from {$tablepre}members where phone='{$phone}' limit 1");
        if ($db->num_rows($query))
            exit("<script>alert('手机号码已经被使用!请换一个手机号码');location.href='register.php'</script>");
...
<div class="logoBar w1000 m0 cf">
    ...
    <form action="?act=reg" method="post" enctype="application/x-www-form-urlencoded" id="regUser" onsubmit="return reg()">
