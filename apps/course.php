...
li{ list-style:none;}
a{ cursor:pointer;}
.courseBox{ background:#fff; width:608px; height:250px; margin:17px auto 0 auto; border-radius:5px; overflow:hidden; padding:12px 12px 0 12px;}
...
<div class="courseBox">
	...
	<div class="course-bottom">
	 <div class="cBox" style="display: none;">
                <ul>
                    <?php
                    $sql="select * from {$tablepre}course where weekid=1 order by paixu asc";
	           $query=$db->query($sql);
            ...
            <div class="cBox" style="display: none;">
                <ul>
                    <?php
                    $sql="select * from {$tablepre}course where weekid=2 order by paixu asc";
	           $query=$db->query($sql);
                    while ($row= $db->fetch_row($query)){
                        ?>
                        <li>
            ...
            <div class="cBox" style="display: none;">
                <ul>
                    <?php
                    $sql="select * from {$tablepre}course where weekid=3 order by paixu asc";
	           $query=$db->query($sql);
                    while ($row= $db->fetch_row($query)){
                        ?>
                        <li>
            ...
            <div class="cBox" style="display: none;">
                <ul>
                    <?php
                    $sql="select * from {$tablepre}course where weekid=4 order by paixu asc";
	           $query=$db->query($sql);
                    while ($row= $db->fetch_row($query)){
                        ?>
                        <li>
            ...
            <div class="cBox" style="display: none;">
                <ul>
                    <?php
                    $sql="select * from {$tablepre}course where weekid=5 order by paixu asc";
	           $query=$db->query($sql);
                    while ($row= $db->fetch_row($query)){
                        ?>
                        <li>
           ...
           <div class="cBox" style="display: none;">
                <ul>
                    <?php
                    $sql="select * from {$tablepre}course where weekid=6 order by paixu asc";
	           $query=$db->query($sql);
            ...
            <div class="cBox" style="display: none;">
                <ul>
                    <?php
                    $sql="select * from {$tablepre}course where weekid=7 order by paixu asc";
	           $query=$db->query($sql);
                   $result=mysql_num_rows($query);
                   if($result==0){ ?>
                     <div style="margin-top: 20px;font-size: 20px;">暂时无课程!</div>
