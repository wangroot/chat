...
<div class="phoneMobile" style="z-index: 9020; display: none;"><h1>为领奖时方便，请输入手机号哦~</h1><div class="registerNewconter"> 手机号：<input type="text" name="mobile" id="mobile" class="mobilemin" maxlength="11" placeholder="请输入手机" title="请输入手机"><a href="javascript:void(0)" id="sendSms" class="btn-send" onclick="rotateSendMsg(this)">获取短信验证码</a></div><div class="registerNewconter registercode">验证码：<input type="text" name="code" id="code" maxlength="100"  placeholder="请输入验证码" title="请输入验证码"></div><button class="buttonBlu" id="reporting" onclick="rotateMobile()">开始抽奖</button><div class="windowMessBtn1" style="right:-15px;top:-15px"></div><div class="windowMessBtn1" style="right:-15px;top:-15px"></div><div class="windowMessBtn1" style="right:-15px;top:-15px"></div><div class="windowMessBtn1" style="right:-15px;top:-15px"></div></div>
<div class="dowebok">
	<div class="rotary">
	...
	<div class="list" id="scrollDiv">
		...
		<ul>
                          <?php
                          $query = $db->query("select * from {$tablepre}rotate  where jiangpin!=''  order by addtime DESC limit 0,20");

while($row=$db->fetch_row($query)){ 
    $phone = substr_replace($row['phone'],'****',3,4);
	$list.="<li><span>{$phone}</span> <span>{$row['jiangpin']}</span></li>";
}
echo $list;
?>
		</ul>
...
$(function(){
  ...
  $('.phoneMobile').css('display','none');
	...
	$hand.click(function(){
      ...
      function hand_run(){
            $.get("rotate.php?act=ismobile",function(data,status){
		 if(data=='noMobile'){
             $('.phoneMobile').show();
			return false;
		 }
		...
		switch(data){
			case 1:
				rotateFunc(1,13,'恭喜你抽中了话费5元');
				break;
			case 2:
				rotateFunc(2,39,'未中奖,再接再厉');
				break;
			case 3:
				rotateFunc(3,64,'恭喜你抽中了话费20元');
				break;
			case 4:
				rotateFunc(4,90,'未中奖,再接再厉');
				break;
			case 5:
				rotateFunc(5,116,'恭喜你抽中了充电宝一个');
				break;
			case 6:
				rotateFunc(6,142,'未中奖,再接再厉');
				break;
			case 7:
				rotateFunc(7,167,'恭喜你抽中了账户赠金1000元');
				break;
			case 8:
				rotateFunc(8,193,'未中奖,再接再厉');
				break;
			case 9:
				rotateFunc(9,219,'恭喜你抽中了暖宝宝一个');
				break;
			case 10:
			...
			case 11:
				...
				break;
			case 12:
				rotateFunc(12,296,'未中奖,再接再厉');
				break;
                        case 13:
				rotateFunc(13,321,'恭喜你抽中了小音响一套');
				break;
			case 14:
...
$.post("/apps/suiji_duanxin.php?action=call_yzm",{static:v},function(data){
				if(data['status'] == 1){
